...
- [ ] **#34 sweep extension to remaining 7 files** (optional) — broad `catch (e: Exception)` instances remain across RecipeRulesRepositoryImpl, NotificationRepositoryImpl, AuthRepositoryImpl, GroceryRepositoryImpl, SettingsRepositoryImpl, PantryRepositoryImpl, StatsRepositoryImpl. Same TDD pattern (RED test → narrow to typed catch → GREEN).
