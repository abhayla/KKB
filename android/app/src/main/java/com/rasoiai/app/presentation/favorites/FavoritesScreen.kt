...
fun FavoritesScreen(
    ...
    // Handle undo snackbar events
    LaunchedEffect(Unit) {
        viewModel.snackbarEvent.collect { event ->
            when (event) {
                is FavoritesSnackbarEvent.ShowUndo -> {
                    val result = snackbarHostState.showSnackbar(
                        message = "${event.recipe.name} removed from favorites",
                        actionLabel = "Undo",
                        duration = SnackbarDuration.Short
                    )
                    if (result == SnackbarResult.ActionPerformed) {
                        viewModel.undoRemoveFromFavorites(event.recipe)
                    }
                }
            }
        }
