...
1. **Bash Path Syntax**: Always use forward slashes `/` in bash commands, not backslashes `\`. Use `./gradlew` not `.\gradlew`. Quote paths with spaces: `cd "D:/Abhay/VibeCoding/KKB/android"`. The shell is Unix-style bash, not Windows CMD.
